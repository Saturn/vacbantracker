...
def test_create_steam_user(setup_app_and_db, mock_steam):
    with mock_steam:
...
def test_get_steam_user(setup_app_and_db, mock_steam):
    with mock_steam:
        steamid = '76561198066693739'
        u = User.get_or_create_steam_user(steamid)
...
def test_is_steam_user(setup_app_and_db, mock_steam):
    with mock_steam:
        steamid = '76561198066693739'
        u = User.get_or_create_steam_user(steamid)
        assert u.steam_user
...
def test_user_track_profile(setup_app_and_db, mock_steam):
    u = User()
    db.session.add(u)
    db.session.commit()
    with mock_steam:
...
def test_user_untrack_profile(setup_app_and_db, mock_steam):
    u = User()
    db.session.add(u)
    db.session.commit()
    with mock_steam:
...
def test_user_untrack_profile_not_tracking(setup_app_and_db, mock_steam):
    u = User()
    db.session.add(u)
    db.session.commit()
    with mock_steam:
...
def test_steam_user_tracking_themselves(setup_app_and_db, mock_steam):
    with mock_steam:
        steamid = '76561198066693739'
        u = User.get_or_create_steam_user(steamid)
...
def test_user_already_tracking_profile(setup_app_and_db, mock_steam):
    u = User()
    db.session.add(u)
    db.session.commit()
    with mock_steam:
